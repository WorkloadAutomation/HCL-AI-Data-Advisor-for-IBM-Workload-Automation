...
build() {
	docker-compose -f $1 build --build-arg REGISTRY="cp.icr.io/cp" --build-arg VERSION=10.2.4
    docker-compose -f $1 up --no-start
}

build_start() {
	docker-compose -f $1 build --build-arg REGISTRY="cp.icr.io/cp" --build-arg VERSION=10.2.4 $2
...
start_config() {
    if ! docker image inspect "cp.icr.io/cp/ibm-workload-automation-aida-config:10.2.4" >&/dev/null; then
